...
snprintf(bash_awk_command,sizeof(bash_awk_command),"sudo /home/pkg/kraken_package_manager/scripts/./extract_dep_with_awk.sh %s %s",pkg_name,SOURCE_DIR);
...
//remove the newline caracter of the line  i mean \n
...
pclose(file);
 ...
 if (*nbr_dep == 0) {
        ...
        fprintf(stderr, "No dependencies found for package: %s\n", pkg_name);

        return NULL; // Return NULL if no dependencies were found
